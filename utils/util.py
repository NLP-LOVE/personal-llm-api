...
# 转换OpenAI流式响应为Claude Messages API流式响应格式
async def openai_to_claude_stream(model, params):
    """
    将 OpenAI 的流式响应转换为 Claude Messages API 的流式响应格式
    """
    message_id = str(snowflake.next_id())
    ...
    async for chunk in model.chat_stream(params):
        ...
        # 3. 发送 content_block_delta (如果有内容)
        if content:
            yield {"type": "content_block_delta", "index": 0, "delta": {"type": "text_delta", "text": content}}
        if reasoning_content:
            yield {"type": "content_block_delta", "index": 0, "delta": {"type": "thinking_delta", "thinking": reasoning_content}}
        # 如果 OpenAI 结束了
        if finish_reason:
            break
    # 4. 发送 content_block_stop
    yield {"type": "content_block_stop", "index": 0}
    # 5. 发送 message_delta (包含结束状态)
    yield {
        "type": "message_delta",
        "delta": {
